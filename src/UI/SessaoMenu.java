...
private void criarSessao() {
    try {
        String dataStr = JOptionPane.showInputDialog("Digite a data da sessão:");
        LocalDate data = LocalDate.parse(dataStr);
        String horarioStr = JOptionPane.showInputDialog("Digite o horário da sessão:");
        ...
        int vagas = Integer.parseInt(
            JOptionPane.showInputDialog("Digite a quantidade de vagas disponíveis:")
        );
        ...
        switch (opcao) {
            case "1":
                LocalDate novaData = LocalDate.parse(
                    JOptionPane.showInputDialog("Nova data:")
                );
                sessao.setData(novaData);
                break;
            case "2":
                LocalTime novoHorario = LocalTime.parse(
                        JOptionPane.showInputDialog("Novo horário: ")
                );
            ...
            case "4":
                ...
                if (novaSala != null) sessao.setSala(novaSala);
                break;
            case "5":
                int novasVagas = Integer.parseInt(
                        JOptionPane.showInputDialog("Novo número de vagas:")
                );
...
private Filme selecionarFilme() {
    ...
    if (filmes.isEmpty()) {
        JOptionPane.showMessageDialog(null, "Nenhum filmecadastrado!");
        return null;
    }
    ...
    try {
        ...
        if (filme == null) {
            JOptionPane.showMessageDialog(null, "Filme nãoencontrado!");
            return null;
        }
        return filme;
    } catch (Exception e) {
